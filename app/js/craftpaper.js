...
$("[data-src='#ordercalc']").on("click", function() {
	...
	for (var i in buildpack.paperOptions) {
		...
		inputContent +=
			'<input class="modal-control" type="hidden" ' +
			'name="' + i + '" ' +
			'value="' + buildpack.paperOptions[i].desc + '" ' +
...
window.checkedToggle = function(checkedInput, valueInput, status) {
	if (status) {
		if ($(checkedInput + ":checked").length != 0) {
			$(valueInput).val("");
			//$(valueInput).siblings("label").addBack().addClass("input-void");
			//$(checkedInput).siblings("label").addBack().removeClass("input-void");
		}
	} else {
		if ($(checkedInput + ":checked").length != 0) {
			$(checkedInput + ":checked")[0].checked = false;
			//$(checkedInput).siblings("label").addBack().addClass("input-void");
			//$(valueInput).siblings("label").addBack().removeClass("input-void");
		}
	}
...
if($(".checkbox-field-customsize").length != 0)
$(".checkbox-field-customsize").on("change", function(){
	$(this).closest(".buildpack-fields").find(".field-customsize").slideToggle();
})
